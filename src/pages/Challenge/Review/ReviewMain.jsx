              ...
              <div className="row gy-4 justify-content-center">
                {subjects.map((subject) => (
                  <div
                    ...
                    style={{ opacity: subject.progressRate === '100.0' ? 1 : 0.4 }}
                  >
                    <div
                      className={`card h-100 text-center p-4 shadow-sm ${
                        subject.progressRate === '100.0' ? 'bg-white' : 'bg-light'
                      }`}
                      onClick={() => subject.progressRate === '100.0' && handleSelect(subject.subject)}
                      style={{ cursor: subject.progressRate === '100.0' ? 'pointer' : 'not-allowed' }}
                    >
                      <h5 className="mb-2">{subject.subject}</h5>
                      <p className="mb-0">진도율: {subject.progressRate}%</p>
