...
const StudyLogPublicDetail = () => {
  ...
  const handleEditSubmit = async (commentId) => {
    try {
      await instance.patch(`/study-diary/comments/${commentId}`, {
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await instance.delete(`/study-diary/comments/${commentId}`);
        ...
        <main className="main" style={{ flex: 1 }} data-aos="fade-up">
          <div className="container py-5">
            ...
            >
              ...
              {editIndex === i ? (
  <div className="ps-3 d-flex align-items-center justify-content-between" style={{ flex: 1 }}>
    {/* 수정 인풋 */}
    <input
      ...
      {c.date ? new Date(c.date).toLocaleDateString() : ''}
              ...
              {openDropdownIndex === i && (
                ...
                >
                  <button
                    className="dropdown-item"
                    onClick={() => handleStartEdit(i, c.contents)}
                    style={{ padding: '8px 12px', width: '100%', border: 'none', background: 'white' }}
                  >
                    수정
                  </button>
